...
首次執行安裝包時,**Windows SmartScreen 可能會彈出「Windows 已保護您的電腦」警告**。這是因為安裝包目前尚未使用付費程式碼簽章憑證,開源專案初期常見,並非病毒。繼續安裝:點擊**其他資訊** → **仍要執行**。

防毒軟體也可能誤報。GGSay 的核心功能是在遊戲中發送訊息,必須**模擬鍵盤輸入**(Ctrl+V、Enter)才能實現貼上與發送。部分防毒軟體的啟發式掃描會將「合成鍵盤按鍵」的行為預設視為可疑。本儲存庫的原始碼完全公開,你可以審查或自行編譯。若被防毒軟體攔截,將 `ggsay.exe` 加入白名單即可。
